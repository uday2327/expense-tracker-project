...
function buildStagedSplits({ row, splitType, normalizedParticipants, rowNumber, rawData, anomalies }) {
  if (!normalizedParticipants.length) {
    return [];
  }

  if (splitType === "EQUAL") {
    return normalizedParticipants.map((member) => ({ userId: member.userId }));
  }

  const valuesByType = {
    EXACT: numericList(row.amounts || row.exact_amounts || row.split_amounts),
    PERCENTAGE: numericList(row.percentages),
    SHARES: numericList(row.shares)
  };
  const values = valuesByType[splitType] || [];

  if (values.length !== normalizedParticipants.length || values.some((value) => Number.isNaN(value))) {
    anomalies.push(anomaly(
      rowNumber,
      rawData,
      "SPLIT_DOESNT_ADD_UP",
      `${splitType} split values must match participant count`,
      "REJECT_ROW"
    ));
    return null;
  }

  return normalizedParticipants.map((member, index) => ({
    userId: member.userId,
    ...(splitType === "EXACT" ? { amount: values[index] } : {}),
    ...(splitType === "PERCENTAGE" ? { percentage: values[index] } : {}),
    ...(splitType === "SHARES" ? { shares: values[index] } : {})
  }));
}
...
export async function createImportReview({ fileText, filename, groupId, uploadedByUserId }) {
  ...
  for (const [index, row] of rows.entries()) {
    ...
    const safeSplitType = ["EQUAL", "EXACT", "PERCENTAGE", "SHARES"].includes(splitType) ? splitType : "EQUAL";
    if (safeSplitType !== splitType) {
      anomalies.push(anomaly(rowNumber, rawData, "MISSING_FIELD", `${splitType} is not a supported split_type`, "DEFAULT_EQUAL_ON_CONFIRM"));
    }
    ...
    const splits = buildStagedSplits({
      row,
      splitType: safeSplitType,
      normalizedParticipants: activeParticipants,
      rowNumber,
      rawData,
      anomalies
    });
