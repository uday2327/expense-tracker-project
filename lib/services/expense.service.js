...
function calculateEqualSplits(participantUserIds, amountInInr) {
  const amounts = splitEvenly(amountInInr, participantUserIds.length);
...
export async function calculateExpenseSplits(data) {
  ...
  if (data.splitType === "EQUAL") {
    const participantUserIds = data.splits?.length
      ? data.splits.map((split) => {
          assertActiveParticipant(split.userId, activeUserIdSet);
          return split.userId;
        })
      : activeUserIds;

    if (!participantUserIds.length) {
      throw appError("Equal splits require at least one participant");
    }

    return calculateEqualSplits(participantUserIds, amountInInr);
  }
